...
* Description: This plugin allows create Multiple Shipping Address Woocommerce plugin.
* Version: 2.0
* Copyright: 2020
* Text Domain: multiple-shipping-address-woocommerce
...
if (!defined('OCWMA_PLUGIN_NAME')) {
  define('OCWMA_PLUGIN_NAME', 'Multiple Shipping Address Woocommerce');
}
  ...
  class OCWMA {
    ...
    function plugin_row_meta( $links, $file ) {
        if (OCWMA_BASE_NAME === $file ) {
            $row_meta = array(
                'rating'    =>  '<a href="https://oceanwebguru.com/multiple-shipping-address-woocommerce/" target="_blank">Documentation</a> | <a href="https://oceanwebguru.com/contact-us/" target="_blank">Support</a> | <a href="https://wordpress.org/support/plugin/multiple-shipping-address-woocommerce/reviews/?filter=5" target="_blank"><img src="'.OCWMA_PLUGIN_DIR.'/images/star.png" class="OCWMA_rating_div"></a>',
            );
...
add_action( 'plugins_loaded', 'OCWMA_load_textdomain' );
function OCWMA_load_textdomain() {
    load_plugin_textdomain( 'multiple-shipping-address-woocommerce', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
}
function OCWMA_load_my_own_textdomain( $mofile, $domain ) {
    if ( 'multiple-shipping-address-woocommerce' === $domain && false !== strpos( $mofile, WP_LANG_DIR . '/plugins/' ) ) {
        $locale = apply_filters( 'plugin_locale', determine_locale(), $domain );
        $mofile = WP_PLUGIN_DIR . '/' . dirname( plugin_basename( __FILE__ ) ) . '/languages/' . $domain . '-' . $locale . '.mo';
    }
    return $mofile;
}
add_filter( 'load_textdomain_mofile', 'OCWMA_load_my_own_textdomain', 10, 2 );